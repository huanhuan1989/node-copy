...
class Base extends Files{
  ...
  fileAndDirNormalize (entryPath, distPath, item) {
		return {name: this.getFilesName(item), value: item.replace(entryPath, distPath), dir: super.isDir(item), file: super.isFile(item)}
  }
