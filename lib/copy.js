...
class Copy extends Base{
	constructor (opts){
		super(opts)
		this.opts = opts
		this.entryPath = path.resolve(opts.entry)
		this.distPath = path.resolve(opts.dist)
		const defOpts = [
			{
				from: '**'
			}
		]
		this.copyPath = super.normalize((opts.copy || defOpts), config.defaultCopyParam) 
		this.init()
	}
	...
	isArrIncludes (str, type = 'file') {
		const result = true
		const fileName = type === 'file' ? str.match(/\.(.*)/) : super.getFilesName(str)
		if(fileName) {
	...
	/**
	 * copyFile  复制文件方法 -- 优化：md5内容是否为一致，如果一致则return
